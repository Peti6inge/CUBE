public class Face
{
    ...
    public void maJEmbrumage(bool isHost) // DONE
    {
        ...
        {
            mouetteCandidate = (
                (Mouette)Jeu.piratitanHost.attaques[Features.AttaqueType.mouette]
            ).getMouette();
        }
        if (!isHost && mouetteClient())
        {
            mouetteCandidate = (
                (Mouette)Jeu.piratitanClient.attaques[Features.AttaqueType.mouette]
            ).getMouette();
        }
    ...
    {
        if (Jeu.piratitanHost.attaques.ContainsKey(Features.AttaqueType.mouette))
        {
    ...
    {
        if (Jeu.piratitanClient.attaques.ContainsKey(Features.AttaqueType.mouette))
        {
            InvocationNonBloquante? mouetteCandidate = (
                (Mouette)Jeu.piratitanClient.attaques[Features.AttaqueType.mouette]
            ).getMouette();
    ...
    {
        if (Jeu.elfeeHost.attaques.ContainsKey(Features.AttaqueType.espritElfique))
        {
            InvocationNonBloquante? espritElfiqueCandidat = (
                (EspritElfique)Jeu.elfeeHost.attaques[Features.AttaqueType.espritElfique]
            ).getEspritElfique();
    ...
    {
        if (Jeu.elfeeClient.attaques.ContainsKey(Features.AttaqueType.espritElfique))
        {
            InvocationNonBloquante? espritElfiqueCandidat = (
                (EspritElfique)Jeu.elfeeClient.attaques[Features.AttaqueType.espritElfique]
            ).getEspritElfique();
            if (espritElfiqueCandidat != null && espritElfiqueCandidat.myCase.face == this)
            {
                return espritElfiqueCandidat;
